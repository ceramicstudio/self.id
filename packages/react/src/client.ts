...
> extends Core<ModelTypes> {
  ...
  constructor(params: WebClientParams<ModelTypes>, session: boolean = false) {
    super(params)
    this.#params = params
  ...
  async authenticate(authProvider: EthereumAuthProvider): Promise<SelfID<ModelTypes>> {
    ...
    const selfID = await SelfID.authenticate<ModelTypes>({ ...this.#params, authProvider, session: this.#session })
